...
# 2) Carrega as credenciais de um arquivo YAML
@st.cache_data
def load_credentials(creds = st.secrets["credentials"]):
    ...
    if erro:
        ...
        for aba, log in zip(abas, logs):
            with aba:
                ...
                mostrar_previsualizacao(log["df_visivel"], nome_log=log["nome"])

                eixo_y = st.multiselect("📊 Selecione até 4 colunas para o gráfico", df_visivel.columns, max_selections=4, key=log["nome"])
                if eixo_y:
